...
class Client extends events_1.EventEmitter {
    ...
    handleAck(req) {
        if (req.options[model_1.OptionId.dhcpMessageType] === model_1.DHCP53Code.DHCPACK) {
            ...
            lastLease.options = new DHCPOptions_1.DHCPOptions();
            ...
            if (req.options[model_1.OptionId.renewalTime]) {
                lastLease.renewPeriod = req.options.get(model_1.OptionId.renewalTime, req);
            }
            // Rebinding time is available
            if (req.options[model_1.OptionId.rebindingTime]) {
                lastLease.rebindPeriod = req.options.get(model_1.OptionId.rebindingTime, req);
            }
            ...
            const cidr = Tools.CIDRFromNetmask(lastLease.options.get(model_1.OptionId.netmask, req));
