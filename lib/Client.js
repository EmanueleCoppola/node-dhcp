...
class Client extends events_1.EventEmitter {
    ...
    sendRequest(req) {
        ...
        this.lastLease.server = req.options.get(model_1.OptionId.server, req.options);
    ...
    handleAck(req) {
        if (req.options[model_1.OptionId.dhcpMessageType] === model_1.DHCP53Code.DHCPACK) {
            ...
            if (req.options[model_1.OptionId.leaseTime]) {
                const leaseTime = req.options.get(model_1.OptionId.leaseTime, req.options);
            ...
            if (req.options[model_1.OptionId.renewalTime]) {
                this.lastLease.renewPeriod = req.options.get(model_1.OptionId.renewalTime, req.options);
            }
            // Rebinding time is available
            if (req.options[model_1.OptionId.rebindingTime]) {
                this.lastLease.rebindPeriod = req.options.get(model_1.OptionId.rebindingTime, req.options);
            }
            ...
            const cidr = Tools.CIDRFromNetmask(this.lastLease.options.get(model_1.OptionId.netmask, req.options));
