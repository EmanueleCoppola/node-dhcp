...
exports.optsMeta = {
    1: {
        config: 'netmask',
        default: function (requested) {
            // Default is the minimal CIDR for the given range
            const range = this.get('range', requested);
    ...
    28: {
        ...
        default: function (requested) {
            const range = this.get('range', requested);
            const netmask = this.get('netmask', requested);
...
exports.attrMapping = {}; // attr name -> id
function addOption(code, opt) {
    exports.optsMeta[code] = opt;
    if (opt.config) {
...
for (const i in exports.optsMeta) {
    addOption(i, exports.optsMeta[i]);
}
